...
{
	public static float Generate(RangePair pair) => Generate2D(pair.mean, pair.std).Item1;
	public static float Generate(float mu, float sigma) => Generate2D(mu, sigma).Item1;
	public static Tuple<float, float> Generate2D(RangePair pair) => Generate2D(pair.mean, pair.std);
